...
Makefile generator and direct compilation tool for MGen projects,
...
class Builder:
    ...
    def _detect_stc_path(self) -> Optional[str]:
        ...
        try:
            # Try to import MGen's STC module from C backend
            import sys
        ...
        possible_paths = [
            ...
            Path.cwd() / "src" / "mgen" / "backends" / "c" / "ext" / "stc" / "include",
...
class MGenMakefileGenerator:
    """MGen-specific Makefile generator with STC integration."""

    def __init__(self, project_name: str = "mgen_project", project_type: str = "MGen"):
        self.log = log.config(self.__class__.__name__)
...
def main() -> int:
    """Command-line interface for MGen Makefile generator."""
    parser = argparse.ArgumentParser(
        description="MGen Makefile Generator with STC support", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
